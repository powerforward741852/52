...
public final class CVCalendarDayView: UIView {
    ...
    public var statuesLab: UILabel!//状态(外出，请假，出差)
    public var localDayLab: UILabel! //农历
...
extension CVCalendarDayView {
    public func labelSetup() {
        ...
        statuesLab = UILabel.init(frame: CGRect.init(x: 33, y: 2.5, width: 15, height: 15))
        statuesLab.textAlignment = .right
        ...
        statuesLab.font = UIFont.systemFont(ofSize: 8)
        addSubview(dayLabel)
        dayLabel.addSubview(statuesLab)
        
        
        localDayLab = UILabel.init()
        let localFreme = CGRect.init(x: 0, y: 33, width: UIScreen.main.bounds.size.width/7, height: 7)
        localDayLab.frame = localFreme
        localDayLab.textAlignment = .center
        localDayLab.font = UIFont.systemFont(ofSize: 7)
        self.addSubview(localDayLab)
    }
    ...
    public func setupDotMarker() {
        ...
        if let delegate = calendarView.delegate {
            if let shouldShow = delegate.dotMarker?(shouldShowOnDayView: self) , shouldShow {
                ...
                var (width, height): (CGFloat, CGFloat) = (6, 6)
                ...
                for (index, color) in (colors!).enumerated() {
                    ...
                    dotMarker.center = CGPoint(x: x, y: y - 5.5)
