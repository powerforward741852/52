...
protocol CQReportSelectDelegate : NSObjectProtocol{
    func pushToDetailThroughType(btn:UIButton)
}

class CQReportSelectView: UIView {

   weak var cqSelectDelegate:CQReportSelectDelegate?
    
    
    lazy var bgImg: UIImageView = {
        let bgImg = UIImageView.init(frame: CGRect.init(x: kWidth - AutoGetWidth(width: 104), y: 64, width: AutoGetWidth(width: 89), height: AutoGetHeight(height: 135)))
        bgImg.image = UIImage.init(named: "CQReportSelect")
        return bgImg
    }()
