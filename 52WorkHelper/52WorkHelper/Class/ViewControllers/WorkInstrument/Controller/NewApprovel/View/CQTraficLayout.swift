...
class CQTraficLayout: TGLinearLayout {
    ...
    convenience init(orientation: TGOrientation,name:String,type:String,title:String,prompt:String,require:Bool,dataSource:[NCQApprovelModel],traficId:String) {
        self.init(frame: CGRect.zero, orientation: orientation, name: name, type: type, title: title, prompt: prompt, require: require,dataSource:dataSource,traficId:traficId )
        
    }
    
    init(frame: CGRect, orientation: TGOrientation,name:String,type:String,title:String,prompt:String,require:Bool,dataSource:[NCQApprovelModel],traficId:String) {
    ...
    func initView()  {
        ...
        if !self.traficId.isEmpty{
            for model in self.modelArray{
                if model.value == self.traficId{
                    self.curSelectTitle = model.text
                }
            }
    ...
    @objc func sureClick() {
        ...
        NotificationCenter.default.post(name: NSNotification.Name.init("traficIdIdValue"), object: self.modelArray[(pickView?.selectedRow(inComponent: 0))!].value)
        self.traficId = self.modelArray[(pickView?.selectedRow(inComponent: 0))!].value
...
//pick代理
extension CQTraficLayout:UIPickerViewDelegate,UIPickerViewDataSource{
