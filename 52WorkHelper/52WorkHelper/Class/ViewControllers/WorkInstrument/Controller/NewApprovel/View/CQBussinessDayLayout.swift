...
class CQBussinessDayLayout: TGLinearLayout {
    ...
    convenience init(orientation: TGOrientation,name:String,type:String,title:String,prompt:String,require:Bool,formula:String,duration:String) {
        self.init(frame: CGRect.zero, orientation: orientation, name: name, type: type, title: title, prompt: prompt, require: require,formula:formula,duration:duration)
        
    }
    
    init(frame: CGRect, orientation: TGOrientation,name:String,type:String,title:String,prompt:String,require:Bool,formula:String,duration:String) {
    ...
    func initView()  {
        ...
        self.addSubview(self.addSelectBtn(title: self.duration ))
    }
...
// Mark 整个界面可能用到的控件
extension CQBussinessDayLayout{
