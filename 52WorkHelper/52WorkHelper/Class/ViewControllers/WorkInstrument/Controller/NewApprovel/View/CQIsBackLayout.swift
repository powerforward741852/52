...
class CQIsBackLayout: TGLinearLayout {
    ...
    convenience init(orientation: TGOrientation,name:String,type:String,title:String,prompt:String,require:Bool,dataSource:[CQBackModel]) {
    ...
    init(frame: CGRect, orientation: TGOrientation,name:String,type:String,title:String,prompt:String,require:Bool,dataSource:[CQBackModel]) {
    ...
    @objc func sureClick() {
        ...
        NotificationCenter.default.post(name: NSNotification.Name.init("backValue"), object: self.modelArray[(pickView?.selectedRow(inComponent: 0))!].value)
        self.isBack = self.modelArray[(pickView?.selectedRow(inComponent: 0))!].value
...
//pick代理
extension CQIsBackLayout:UIPickerViewDelegate,UIPickerViewDataSource{
