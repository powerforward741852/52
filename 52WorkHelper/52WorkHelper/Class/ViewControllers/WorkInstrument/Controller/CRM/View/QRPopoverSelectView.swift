...
import UIKit
protocol QRPopoverSelectViewDelegate : NSObjectProtocol{
    func pushToDetailThroughType(btn:UIButton)
}
class QRPopoverSelectView: UIView {
    weak var cqSelectDelegate:QRPopoverSelectViewDelegate?
    lazy var bgImg: UIImageView = {
        let bgImg = UIImageView.init(frame: CGRect.init(x: kWidth - AutoGetWidth(width: 104), y: 64, width: AutoGetWidth(width: 89), height: AutoGetHeight(height: 95)))
