...
import UIKit
protocol QRFileCellSelectDelegate : NSObjectProtocol{
    func selectIndex(index:IndexPath)
}
class QRFileCell: UITableViewCell {

    weak var selectDelegate:QRFileCellSelectDelegate?
    ...
    lazy var docNameLab: UILabel = {
        let docNameLab = UILabel.init(frame: CGRect.init(x: self.iconImg.right + AutoGetWidth(width: 5), y: AutoGetHeight(height: 11.5), width: kWidth*2/3, height: AutoGetHeight(height: 16)))
    ...
    //定义模型属性
    var model: QRFileModel? {
        didSet {
            self.docNameLab.text = model?.attachmentOldName
            self.masterNameAndDateLab.text = model?.createTime
