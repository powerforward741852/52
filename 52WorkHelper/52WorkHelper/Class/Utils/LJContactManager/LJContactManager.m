...
- (void)createNewContactWithPhoneNum:(LJPerson*)person controller:(UIViewController *)controller
{
    if (IOS9_OR_LATER)
    {
          CNMutableContact *contact = [[CNMutableContact alloc] init];
        //电话
        NSMutableArray*mutiPhone = [NSMutableArray new];
        for (int i= 0; i<person.phones.count; i++) {
            CNLabeledValue *labelValue = [CNLabeledValue labeledValueWithLabel:person.phones[i].label 
                                                                         value:[CNPhoneNumber phoneNumberWithStringValue:person.phones[i].phone]];
            [mutiPhone addObject:labelValue];
            
        }
        contact.phoneNumbers = mutiPhone.copy;
        //地址
        NSMutableArray*mutiAddress = [NSMutableArray new];
        for (int i= 0; i<person.addresses.count; i++) {
            CNMutablePostalAddress* mu = [CNMutablePostalAddress new];
            mu.street =  person.addresses[i].street;
            CNLabeledValue *labelValue = [CNLabeledValue labeledValueWithLabel:CNLabelWork value:mu];
            [mutiAddress addObject:labelValue];
        }
        contact.postalAddresses = mutiAddress;
        
        //网址
        NSMutableArray*mutiUrlStr = [NSMutableArray new];
        for (int i= 0; i<person.urls.count; i++) {
            CNLabeledValue *labelValue = [CNLabeledValue labeledValueWithLabel:CNLabelWork value:person.urls[i].urlString];
            [mutiUrlStr addObject:labelValue];
        }
        contact.urlAddresses = mutiUrlStr;
        
        //email
        NSMutableArray*mutiEmails = [NSMutableArray new];
        for (int i= 0; i<person.emails.count; i++) {
            CNLabeledValue *labelValue = [CNLabeledValue labeledValueWithLabel:CNLabelWork value:person.emails[i].email];
            [mutiEmails addObject:labelValue];
        }
        contact.emailAddresses = mutiEmails;
        
        contact.nickname = person.fullName;
...
- (void)saveNewContactWithPhoneNum:(LJPerson*)person
{
    CNMutableContact *contact = [[CNMutableContact alloc] init];
    //电话
    NSMutableArray*mutiPhone = [NSMutableArray new];
...
- (NSArray *)queryContactWithName:(NSString *)name{
    ...
    //@[CNContactEmailAddressesKey, [CNContactFormatter descriptorForRequiredKeysForStyle:CNContactFormatterStyleFullName]];
    NSArray *contact = [store unifiedContactsMatchingPredicate:predicate keysToFetch:keysToFetch error:nil];
    return contact;
}
- (BOOL)deleteContacts:(NSString*)familyName{
    
    NSArray *contact = [self queryContactWithName:familyName];
    if (contact.count>0){
        //        for (int i= 0; i<contact.count; i++) {
        //
        //        }
        // 创建联系人请求
        CNMutableContact*muti = [contact.lastObject mutableCopy];
        CNSaveRequest *saveRequest = [[CNSaveRequest alloc] init];
        [saveRequest deleteContact:muti];
        // 写入操作
        CNContactStore *store = [[CNContactStore alloc] init];
        [store executeSaveRequest:saveRequest error:nil];
        return YES;
